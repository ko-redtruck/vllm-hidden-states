...
class LLMEngine:
    ...
    def __init__(
        ...
        logger.info(
            "Hello Initializing an LLM engine (v%s) with config: %s, "
    ...
    def _process_model_outputs(self,
        ...
        if has_multiple_outputs:
            ...
            for sampler_output in outputs:
                for i, completion_sequence_group_output in enumerate(sampler_output.outputs):
                    if len(completion_sequence_group_output.samples) == 0:
                        empty_skips += 1
                        continue 
                    try:
                        completion_sequence_group_output.hidden_state = (sampler_output.hidden_states[i - empty_skips, :].clone().cpu().unsqueeze(0))
                    except:
                        print("error on setting hidden state")
        ...
        for i in indices:
            ...
            try:
                seq_group.hidden_states = (torch.cat([seq_group.hidden_states, output[0].hidden_state]) if seq_group.hidden_states is not None else output[0].hidden_state)
            except:
                print("seq_group.hidden_states accumalation error")
