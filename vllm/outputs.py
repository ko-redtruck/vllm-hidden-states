...
class CompletionOutput:
    ...
    def __repr__(self) -> str:
        return (f"CompletionOutput(index={self.index}, "
                ...
                "hidden_states="
                f"Tensor({self.hidden_states.shape if self.hidden_states is not None else "None)"}))"
                )
        ...
        for i, seq in enumerate(top_n_seqs):
            ...
            if use_cache:
                ...
                output = CompletionOutput(
                    ...
                    seq.stop_reason, None, None)
            
            # Should be seq instead of seq_group. Might not work if you use beam search or somethling that with more than one sequence per group
